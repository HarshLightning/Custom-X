...
<body>
	...
	<nav class="navbar navbar-expand-sm container my-3">
		...
		<div class="collapse navbar-collapse" id="my-navbar">
			<ul class="navbar-nav mx-auto p-2">
				...
				<li><a href="UserOrders.jsp">Order</a></li>
				<li><a href="BuildPC.jsp">Build My PC</a></li>
			</ul>
			<label id="call" href=""> Welcome: <b><%= name %></b> <%
            		DAO	db=new DAO();
            		int count=db.countCart(email);
            		%> <a class="btn btn-sm btn-dark" href="Cart.jsp">Cart
					[<%=count %>]
			</a> <a class="btn btn-sm btn-danger" href="Logout">Logout</a>
			</label>
		</div>
	</nav>
    ...
    %>
		<div class="bg-warning p-2 m-2">
			<p>
				ID: <b><%= orderid  %></b> Status: <b><%= status  %></b>
				Date: <b><%= order.get("order_date")  %></b>
				<%
 							if(status.equalsIgnoreCase("Pending") || status.equalsIgnoreCase("Accepted")){
 							%>
				<a
					href="ChangeOrderStatus?id=<%= order.get("id") %>&email=<%= email %>&status=Canceled&page=UserOrders"
					class="btn btn-sm btn-danger mx-2">Cancel</a>
				<%	
 							}
 							%>
			</p>
			      ...
			      <div class="modal-body">
			        ...
			        <input type="hidden" name="orderId" value="<%= orderid %>">
			          <div class="form-group">
			            ...
			            <textarea class="form-control" id="feedbackTextarea"
		...
		<div class="bg-warning p-2 m-2">
			<p>
				ID: <b><%= order.get("id")  %></b> Status: <b><%= status  %></b>
				Date: <b><%= order.get("order_date")  %></b>
				<%
 							if(status.equalsIgnoreCase("Pending") || status.equalsIgnoreCase("Accepted")){
 							%>
    ...
    <label id="top-button"><i
		class="fa-solid fa-circle-up fa-2x"></i></label>
